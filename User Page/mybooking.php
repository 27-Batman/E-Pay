...
if (isset($_POST['submit_payment'])) {
    ...
    $check_query = "SELECT * FROM payments WHERE sc_no = '$sc_no'";
    $result = $conn->query($check_query);

    if ($result->num_rows > 0) {
        echo "<script>alert('Error: Duplicate entry! This SC No already exists.');</script>";
    } else {
        $payment_query = "INSERT INTO payments (sc_no, due_date, bill_amt, rebate_fine, payable_amt) 
                          VALUES ('$sc_no', '$due_date', '$bill_amt', '$rebate_fine', '$payable_amt')";
...
<head>
    ...
    <script>
        function updatePayableAmount() {
            // Get bill amount, rebate/fine, and payable amount elements
            ...
            // Calculate payable amount (Bill Amount + Rebate/Fine)
            var payableAmount = billAmount + rebateFine;

            // Set the calculated payable amount to the payable_amt field
